...
def search(
    ...
    try:
        ...
        db.search(query=query, since=since, until=until, limit=limit, limit_per_database=limit_per_database,
                  databases=databases, publication_types=publication_types, scopus_api_token=scopus_api_token,
                  ieee_api_token=ieee_api_token, proxy=proxy, verbose=verbose)
...
def refine(
    ...
    try:
        ...
        db = paperplumber.FindPapersDatabase(path=path)
        db.refine(categories=categories_by_facet, highlights=highlights, show_abstract=show_abstract,
                  show_extra_info=show_extra_info,
                  only_selected_papers=only_selected_papers, only_removed_papers=only_removed_papers,
...
def download(
    ...
    try:
        ...
        for categories_string in categories:
            ...
            categories_by_facet[facet] = [x.strip() for x in string_split[1].split(',')]
        db = paperplumber.FindPapersDatabase(path=path)
        db.download(only_selected_papers=only_selected_papers, categories_filter=categories_by_facet, proxy=proxy,
                    verbose=verbose)
...
def list_available(
    ...
    try:
        ...
        for paper in papers:
            table.add_row(paper['publication_date'], paper['title'], ",".join(paper['authors']))
        console = Console()
        console.print(table)
